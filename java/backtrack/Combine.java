...
public class Combine {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("combined: " + c.combine(n, k));
    }

    public List<List<Integer>> combine(int n, int k) {
        List<List<Integer>> lists = new ArrayList<>();
        comb(lists, new ArrayList<>(), n, 1, k);
        return lists;
    }
     ...
     * 数字个数达到k个则是一个合法子集
     */
    private void comb(List<List<Integer>> lists, List<Integer> list, int n, int start, int k) {
        if (k == 0) {

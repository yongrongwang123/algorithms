...
public class SmallestStringWithSwaps {
    ...
    public static void main(String[] args) {
        ...
        for (int[] a : arr) {
            pairs.add(a1.toList(a));
        }
        System.out.println("s: " + s);
        System.out.println("pairs: " + pairs);
        SmallestStringWithSwaps s1 = new SmallestStringWithSwaps();
        System.out.println("s: " + s1.smallestStringWithSwaps(s, pairs));
    }
    ...
    public String smallestStringWithSwaps(String s, List<List<Integer>> pairs) {
        ...
        Map<Integer, ArrayList<Character>> graph = createGraph(s, uf);
        for (List<Character> neighbors: graph.values()) {
            Collections.sort(neighbors, (a, b) -> b - a);
        }
        for (int i = 0; i < n; i++) {
            List<Character> neighbors = graph.get(uf.find(i));
            builder.append(neighbors.removeLast());
        }
        return builder.toString();
    }

    private Map<Integer, ArrayList<Character>> createGraph(String s, UnionFind uf) {
        int n = s.length();
        char[] chars = s.toCharArray();
        Map<Integer, ArrayList<Character>> graph = new HashMap<>();
        for (int i = 0; i < n; i++) {
            int root = uf.find(i);
            graph.putIfAbsent(root, new ArrayList<>());
            graph.get(root).add(chars[i]);
        }
        return graph;
    }

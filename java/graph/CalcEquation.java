...
public class CalcEquation {
    ...
    public static void main(String[] args) {
        ...
        for (String[] a : arr1) {
            equations.add(Arrays.asList(a));
        }
        List<List<String>> queries = new ArrayList<>();
        for (String[] a : arr2) {
            queries.add(Arrays.asList(a));
        }
        ...
        System.out.println("answers: " + 
                Arrays.toString(c.calcEquation(equations, values, queries)));
    }
    ...
    public double[] calcEquation(List<List<String>> equations, double[] values,
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (graph.containsKey(start) && graph.containsKey(end)) {
                answer = visitGraph(start, end, graph, new HashSet<>());
            }
            answers[i] = answer;
        }
        return answers;
    ...
    private Map<String, Map<String, Double>> createGraph(List<List<String>> equations,
            double[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            graph.get(u).put(v, radio);
            graph.get(v).put(u, 1 / radio);
        }
        return graph;
    }
     ...
     */
    private double visitGraph(String start, String end, Map<String,
            Map<String, Double>> graph, Set<String> visited) {
        visited.add(start);
        ...
        for (String next : neighbors.keySet()) {
            ...
            double weight = visitGraph(next, end, graph, visited);
            if (weight != -1) {

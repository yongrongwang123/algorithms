...
var smallestStringWithSwaps = function(s, pairs) {
    ...
    let graph = createGraph(s, uf);
    for (let neighbors of graph.values()) {
        neighbors.sort((a, b) => b - a);
    }
    for (let i = 0; i < n; i++) {
        let neighbors = graph.get(uf.find(i));
        builder += neighbors.pop();
    }
    return builder;
}

var createGraph = function(s, uf) {
    let n = s.length;
    let graph = new Map();
    for (let i = 0; i < n; i++) {
        let root = uf.find(i);
        graph.set(root, graph.get(root) ?? []);
        graph.get(root).push(s[i]);
    }
    return graph;
};
...
var main = function() {
    ...
    console.log('s: ' + smallestStringWithSwaps(s, pairs));
}

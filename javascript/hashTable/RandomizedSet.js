...
RandomizedSet.prototype.remove = function(val) {
    ...
    let cur = this.numToIndex.get(val);
    if (cur != last) {
        let lastNum = this.nums[last];
        this.numToIndex.set(lastNum, cur);
        this.nums[cur] = lastNum;
    }
    this.nums.pop();
    this.numToIndex.set(val, null);
    return true;
}
...
var main = function() {
    ...
    console.log('insert ' + arr[0] + ': ' + set.insert(arr[0]));
    console.log('remove ' + arr[1] + ': ' + set.remove(arr[1]));
    console.log('insert ' + arr[1] + ': ' + set.insert(arr[1]));
    console.log('random: ' + set.getRandom());
    console.log('remove ' + arr[0] + ': ' + set.remove(arr[0]));
    console.log('insert ' + arr[1] + ': ' + set.insert(arr[1]));
    console.log('random: ' + set.getRandom());
}

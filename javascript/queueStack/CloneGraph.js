...
var createGraph = function(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        map.set(i + 1, map.get(i + 1) ?? new _Node(i + 1));
        node = map.get(i + 1);
        let neighbor = null;
        for (let j = 0; j < arr[i].length; j++) {
            let key = arr[i][j];
            map.set(key, map.get(key) ?? new _Node(key));
...
var printNeighbors = function(len, map) {
    ...
    for (let i = 0; i < len; i++) {
        let neighbors = map.get(i + 1).neighbors;
        str += '\n';
...
var main = function() {
    ...
    let map = new Map();
    putMap(len, node, map);
    printNeighbors(len, map);
    let newNode = cloneGraph(node);
    let newMap = new Map();
    putMap(len, newNode, newMap);
    printNeighbors(len, newMap);
}

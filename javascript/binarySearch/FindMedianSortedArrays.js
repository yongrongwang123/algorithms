...
var findMedianSortedArrays = function(nums1, nums2) {
    let n1 = nums1.length;
    let n2 = nums2.length;
    let med1 = getKth(nums1, 0, nums2, 0, parseInt((n1 + n2 + 1) / 2));
    if (!((n1 + n2) % 2)) {
        let med2 = getKth(nums1, 0, nums2, 0, parseInt((n1 + n2 + 2) / 2));
        return (med1 + med2) / 2;
    }
    return med1;
}
...
var getKth = function(nums1, start1, nums2, start2, k) {
    ...
    let mid1 = start1 + parseInt(k / 2) - 1;
    let mid2 = start2 + parseInt(k / 2) - 1;
    ...
    if (med1 < med2) {
        return getKth(nums1, mid1 + 1, nums2, start2, k - parseInt(k / 2));
    } else {
        return getKth(nums1, start1, nums2, mid2 + 1, k - parseInt(k / 2));
    }
}

...
var maximumGap = function(nums) {
    ...
    let pre = gap;
    let cur = pre;
    let minBucket = new Array(n).fill(1000000001);
    let maxBucket = new Array(n).fill(-1);
    for (let num of nums) {
        let i = Math.floor((num - min) / gap);
        minBucket[i] = (minBucket[i] <= num ? minBucket[i] : num);
        maxBucket[i] = (maxBucket[i] >= num ? maxBucket[i] : num);
    }
    for (let i = 1, j = maxBucket[0]; i < n; i++) {
        if (maxBucket[i] != -1) {
            cur = minBucket[i] - j;
            pre = (pre >= cur ? pre : cur);
            j = maxBucket[i];

...
var smallestTrimmedNumbers = function(nums, queries) {
    ...
    let n = nums.length;
    let n0 = nums[0].length;
    let answers = new Array(m);
    let list = new Array(n);

    for (let i = 0; i < nums.length; i++) {
        list[i] = [nums[i], i];
    }
    for (let i = n0 - 1, k = 0; i >= 0 && k < m; i--) {
        list = countingSort(list, i);
        for (let j = 0; j < m && k < m; j++) {
            if (n0 - queries[j][1] == i){
                answers[j] = list[queries[j][0] - 1][1];
                k++;
            }
        }
    }
 ...
 * 作为新数组的索引
 */
var countingSort = function(nums, digit) {
    let counts = new Array(10).fill(0);
    let sorted = new Array(nums.length);

    for (let num of nums) {
        let index = num[0].charAt(digit) - '0';
        counts[index]++;
    }
    ...
    for (let i = nums.length - 1; i >= 0; i--) {
        let index = nums[i][0].charAt(digit) - '0';
        counts[index]--;
...
console.log('answers: ' + smallestTrimmedNumbers(nums, queries));

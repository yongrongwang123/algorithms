 ...
 * 频率更大，则该字符串一定不能成功检索到
 */
var checkFreq = function(board, word) {
    let a = 'A'.charCodeAt(0);
    let freq = new Array(58).fill(0);
    for (let i = 0; i < board.length; i++) {
        for (let j = 0; j < board[0].length; j++) {
            freq[board[i][j].charCodeAt(0) - a]++;
        }
    }
    for (let c of word) {
        freq[c.charCodeAt(0) - a]--;
        if (freq[c.charCodeAt(0) - a] < 0) {
            return false;
        }
    }
    return true;
 ...
 */
var helper = function(board, row, col, word, start) {
